...
void CU(int sig) {
    ...
    if (sig == SIGALRM) { 
        ...
        if (cache_read(instructCounter, &value) != 1) {
        ...
        switch (command) {
            ...
            case 0x0B:
                ...
                // if (cache_read(address, &val) == 1) // ! CACHE
                //     printblack(27, 90, "cache line is hit");
                cache_read(address, &val);
                printbl(28, 2);
            ...
            case 0x33:
                ...
                if (cache_write(instructCounter, ~ac_value) == -1) 
